...
def all_jobs():
    ...
    all_jobs_summary = Label(all_jobs_display, text=jobs_list)
    all_jobs_summary.grid(row=0)

    all_jobs_buttons_frame = Frame(all_jobs_frame)
    all_jobs_buttons_frame.grid(row=3)

    previous_button = Button(all_jobs_buttons_frame, text="Previous", state=DISABLED, command=lambda:previous(counter, jobs_list))
    previous_button.grid(row=0, column = 0)

    next_button = Button(all_jobs_buttons_frame, text="Next", command=lambda:previous(counter, jobs_list))
    next_button.grid(row=0, column = 1)

    if len(jobs_list) == 1:
        next_button.config(state=DISABLED)
def next(counter, jobs_list):
    counter += 1
    all_jobs_summary.config(text=jobs_list[counter])
    check(jobs_list)

def previous(counter, jobs_list):
    counter -=1
    all_jobs_summary.config(text=jobs_list[counter])
    check(jobs_list)
...
def check(jobs_list):
    if counter < len(jobs_list) -1:
        next_button_config(state=NORMAL)
    if counter == len(jobs_list) -1:
        next_button.config(state=DISABLED)
    if counter > 0:
